...
plot_doorsign <- function(dfr, pos_x = 0.12, pos_y_text = 0.56, line_spacing = 0.042, canvas_height = 210, canvas_width = 297,
  ...
  if (!is.null(logo_right)) {
    ...
    if (!debug) {
      p <- p + annotation_raster(logo_right,
        xmin = dims_logo_right$xmin, xmax = dims_logo_right$xmax,
    ...
    if (debug) {
      p <- p + annotate("rect",
        ...
        annotate("text",
          x = dims_logo_right$xmin + ((dims_logo_right$xmax - dims_logo_right$xmin) / 2),
          y = dims_logo_right$ymin + ((dims_logo_right$ymax - dims_logo_right$ymin) / 2),
          label = paste0(
            "raw: x", logo_right_offset_x, ", y", logo_right_offset_y, "\n",
  ...
  if (!is.null(logo_left)) {
    ...
    if (!debug) {
      p <- p + annotation_raster(logo_left,
        xmin = dims_logo_left$xmin, xmax = dims_logo_left$xmax,
    ...
    if (debug) {
      p <- p + annotate("rect",
        ...
        annotate("text",
          x = dims_logo_left$xmin + ((dims_logo_left$xmax - dims_logo_left$xmin) / 2),
          y = dims_logo_left$ymin + ((dims_logo_left$ymax - dims_logo_left$ymin) / 2),
          label = paste0(
            "raw: x", logo_left_offset_x, ", y", logo_left_offset_y, "\n",
  ...
  if (!is.null(im_profile)) {
    ...
    if (!debug) {
      p <- p + annotation_raster(im_profile,
        xmin = dims_im_profile$xmin, xmax = dims_im_profile$xmax,
    ...
    if (debug) {
      p <- p + annotate("rect",
        ...
        annotate("text",
          x = dims_im_profile$xmin + ((dims_im_profile$xmax - dims_im_profile$xmin) / 2),
          y = dims_im_profile$ymin + ((dims_im_profile$ymax - dims_im_profile$ymin) / 2),
          label = paste0(
            "raw: x", im_profile_offset_x, ", y", im_profile_offset_y, "\n",
  ...
  if (!is.null(logo_email)) {
    ...
    for (i in 1:nrow(dfr_email)) {
      ...
      if (!debug) {
        p <- p + annotation_raster(logo_email,
          xmin = dims_logo_email$xmin, xmax = dims_logo_email$xmax,
      ...
      if (debug) {
        p <- p + annotate("rect",
          ...
          annotate("text",
            x = dims_logo_email$xmin + ((dims_logo_email$xmax - dims_logo_email$xmin) / 2),
  ...
  if (!is.null(logo_phone)) {
    ...
    for (i in 1:nrow(dfr_phone)) {
      ...
      if (!debug) {
        p <- p + annotation_raster(logo_phone,
          xmin = dims_logo_phone$xmin, xmax = dims_logo_phone$xmax,
      ...
      if (debug) {
        p <- p + annotate("rect",
          ...
          annotate("text",
            x = dims_logo_phone$xmin + ((dims_logo_phone$xmax - dims_logo_phone$xmin) / 2),
